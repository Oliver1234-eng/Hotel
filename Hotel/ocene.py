def sortiranje_liste_ocena(lista_ocena):
    return sorted(lista_ocena, key = lambda ocena: ocena['naziv'])

def dodavanje_nove_ocene(lista_ocena, nova_ocena):
    lista_ocena.append(nova_ocena)

    return True

def zaglavlje():
    print("id    |naziv hotela   |ocena          ")
    print("--------------------------------------")

def prikaz_ocena(ocena):
    print(str(ocena['id']).ljust(6) + "|" + ocena['naziv'].ljust(15)[:15] + "|" + str(ocena['ocena']).ljust(15))

def prikaz_svih_ocena(lista_ocena):
    for ocena in lista_ocena:
        prikaz_ocena(ocena)
